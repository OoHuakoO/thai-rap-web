...
export function TimelineArea({ storeId, round, assessmentId, notes }: TimelineAreaProps) {
  ...
  const items = ROUNDS.filter((r) => r === round || summaries?.some((s) => s.round === r)).map(
    (r) => {
      ...
      return {
        round: r,
        title: isCurrent ? `รอบปัจจุบัน ${r}` : ROUND_LABELS[r],
  ...
  return (
    <div className="rounded-xl border bg-card p-3 shadow-sm">
      <div className="mb-2 flex items-center justify-between">
        <p className="text-xs font-bold text-charcoal">บันทึกและประวัติการประเมิน</p>
        <p className="text-[10px] text-muted-foreground">บันทึกเพิ่มเติมเกี่ยวกับผู้ประเมิน</p>
      </div>
      ...
      <div className="flex flex-wrap items-start gap-2">
          ...
          {items.map((item, i) => (
            <div key={item.round} className="flex items-center gap-2">
              ...
              >
                ...
                {item.date && <p className="mt-0.5 text-[9px] text-muted-foreground">{formatDate(item.date)}</p>}
                <span
                  className={cn(
                ...
                >
                  {item.status === 'current' ? 'กำลังประเมิน' : item.status === 'done' ? 'เสร็จสิ้น' : 'บันทึกร่าง'}
                </span>
              </div>
              {i < items.length - 1 && <ArrowRight className="h-3.5 w-3.5 flex-shrink-0 text-border" />}
            </div>
          ))}
        </div>
        ...
        <div className="min-w-[190px] max-w-[260px] rounded-lg border-l-[3px] border-l-orange bg-muted/30 p-2.5">
          <div className="mb-1 flex items-center justify-between">
            <p className="text-[10.5px] font-bold text-charcoal">บันทึกเพิ่มเติม</p>
            {!editingNotes && (
              <button
                type="button"
          ...
          {editingNotes ? (
            ...
            <p className="text-[10.5px] leading-relaxed text-muted-foreground">
              {notesValue || 'ยังไม่มีบันทึกเพิ่มเติม'}
            </p>
          )}
        </div>
      </div>
