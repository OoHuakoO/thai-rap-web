...
export function StoreCoverManager({ storeId, coverUrl }: StoreCoverManagerProps) {
  const inputRef = useRef<HTMLInputElement>(null);
  const { mutate: uploadCover, isPending: isUploading } = useUploadStoreCover(storeId);
  ...
  return (
    <div className="flex flex-shrink-0 flex-col items-center gap-2 rounded-xl border-2 border-dashed border-orange/30 bg-orange/5 p-4 sm:w-72">
      ...
      <button
        ...
        className="group relative flex h-36 w-full flex-shrink-0 items-center justify-center overflow-hidden rounded-xl border-4 border-card bg-muted shadow-md ring-2 ring-orange/20 disabled:opacity-50"
      >
        {coverUrl ? (
